...
The next student push or merge request pipeline will run the hidden tests.
...
1. Student pushes code or opens a merge request.
...
For a different course, update the defaults in:

```text
Student Exercise Repo Template/.gitlab-ci.yml
Private Tester/.gitlab-ci.yml
```
...
COURSE_NAME
COURSE_PATH
PRIVATE_TESTER_PROJECT_PATH
PYTHON_AUTO_TESTER_PROJECT_PATH
```

For the current course, leave them as they are.
